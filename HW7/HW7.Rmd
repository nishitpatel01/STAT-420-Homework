...
Since the p-value from ANOVA test is significant compare to given $\alpha$ value, we prefer interaction model.
...
```
  ...
  - Value of test statistic is `r anova(fish_smaller, fish_bigger)[2,5]`.
  - p-value of the test is `r anova(fish_smaller, fish_bigger)[2,6]`.
  - Since p-value of the test is very small compare to $\alpha$ value, we would reject the null hypothesis.
...
```
  ...
  Based on above expression, we can see that coefficient of $x_1$ depends on linear function of $\beta$ as well as predictors $x_2$ i.e. `HeightPct` and $x_3$ i.e. `WidthPct`.
...
In this case the coefifcient of $x_1$ i.e. `Length1` is $\beta_1$. It does not depend on any other $\beta$ value and other predictors. 
...
When looking at the first 10 elements of `lm_t` and `tt_t`, we see a pattern where each of the value from `lm_t` are of opposite sign from `tt_t`. This is because for `lm_t` the `lm` method when `R`created $x_1$ (`Gropus` in this case), the dummy variable, it used `Groups A` as a reference level which is default value of factor variable so when dummy variable is 0, the model represents reference level, which is `Group A` in this case. 
